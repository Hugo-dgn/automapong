...
def create(args):
    ...
    if args.type == "ql":
        ...
        agent = agents.DeepQLearningAgent(args.agent, DQN=DQN, lr=args.lr, gamma=args.gamma, eps=args.eps, end_eps=args.eeps, eps_decay=args.edecay, capacity=args.capacity, batch=args.batch, skip=args.skip, update=args.update)
        agent.save()
...
def main():
    ...
    create_parser.add_argument("-eps", type=float, help="Epsilon parameter for epsilon greedy algorithm.", default=0.3)
    create_parser.add_argument("-eeps", type=float, help="End epsilon parameter for epsilon greedy algorithm.", default=0.01)
    create_parser.add_argument("-edecay", type=float, help="Exponential decay parameter for epsilon greedy algorithm.", default=0.0001)
